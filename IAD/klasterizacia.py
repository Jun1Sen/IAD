...
def klasterizacia():
    ...
    # Выводим статистику по каждому кластеру
    df['Cluster'] = cluster_labels
    cluster_stats = df.groupby('Cluster')[['Gender', 'Model']].describe()
    print(cluster_stats)

    # Выводим средние значения признаков для каждого кластера
    ...
    # Создаем scatter plot для каждого кластера с использованием Seaborn
    plt.figure(figsize=(10, 6))
    sns.scatterplot(data=cluster_0, x='Gender', y='Model', color='blue', label='Кластер №1', legend=False)
    sns.scatterplot(data=cluster_1, x='Gender', y='Model', color='green', label='Кластер №2', legend=False)
    sns.scatterplot(data=cluster_2, x='Gender', y='Model', color='purple', label='Кластер №3', legend=False)
    sns.scatterplot(data=cluster_3, x='Gender', y='Model', color='red', label='Кластер №4', legend=False)

    # Получаем исходные категориальные данные для модели
    models_original = df[df['Cluster'] == 0]['Vehicle_Type']

    # Выводим распределение типов ТС
    ...
    # Вывод данных о ТС в кластере 'Прочие марки и модели ТС' на консоль
    print(other_models)
